...
angular
  ...
  .controller('plotter', [
    ...
    'debounce',
    'NgMap',
    function($scope, $filter, debounce, NgMap) {
      $scope.markers = []
      $scope.clusters = []
      $scope.map = {}
      ...
      $scope.onMapOverlayCompleted = function(e) {
        function getMarkersInBounds(markers, polygon) {
        ...
        var cluster = {
          polygon: polygon,
          markers: getMarkersInBounds($scope.markers, polygon)
        }

        // update the cluster if the polygon is edited or moved
        polygon.onCoordinatesChanged(function() {
          cluster.markers = getMarkersInBounds($scope.markers, polygon)
          $scope.$apply()
        })
